...
router.post("/report-problem", (req, res) => {
    ...
    Problem.find(query)
        ...
        .exec((err, problem) => {
            ...
            // ใช้ฟอนต์ภาษาไทย
            doc.font('public/fonts/THSarabunNew Bold.ttf').fontSize(24).text('รายงานข้อมูลการแจ้งปัญหาในการประชุม', {
                align: 'center'
            });

            // Table header
            const tableData = [
                ['#', 'ชื่อปัญหา', 'ชื่อผู้แจ้ง', 'วันที่แจ้ง', 'รายละเอียด', 'การแก้ปัญหา', 'สถานะ']
            ];
            ...
            problem.forEach(problem => {
                ...
                let formattedDate = problem.Date ? new Date(problem.Date).toLocaleDateString('en-GB', {
                    day: '2-digit',
                    month: '2-digit',
                    year: '2-digit'
                }) : '';
                ...
                tableData.push([
                    rowNumber++,
                    problem.ProblemName,
                    problem.UserID ? problem.UserID.Prefix + ' ' + problem.UserID.FirstName + ' ' + problem.UserID.LastName : '',
                    formattedDate,
            ...
            const columnWidths = [20, 60, 130, 80, 75, 75, 70]; // คอลัมน์ที่ 1, 2 และ 3 ตามลำดับ
